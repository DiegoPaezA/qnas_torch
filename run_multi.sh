...
for ((i=1; i<=5; i++)); do
    exp_path="multi_50_exp_adamw_$i"
...
for ((i=1; i<=5; i++)); do
    exp_path="multi_100_exp_adamw_$i"
...
# Run the evolution experiment multiple times with different config files rmsprop
echo "Running evolution experiment with config5.txt"

for ((i=1; i<=5; i++)); do
    exp_path="multi_100_exp_rms_$i"
    
    python run_evolution.py \
        --experiment_path "$exp_path" \
        --config_file 'config_files/config5.txt' \
        --data_path cifar10_data \
        --log_level INFO
done